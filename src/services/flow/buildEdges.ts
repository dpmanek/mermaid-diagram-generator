...
import { handlesForNodeRelationship, handlesForRelationship } from "./handles";
import { inferGroupRelationships } from "./inferredEdges";

export function buildFlowEdges(model: ArchitectureModel, themeId: ThemeId, visualSettings: VisualSettings): Edge[] {
  ...
  const parsedEdges = model.edges.flatMap((edge) => {
    ...
    const handles = bothGroups
      ? handlesForRelationship(source, target, groupBounds)
      : handlesForNodeRelationship(source, target, nodesById);
    return [decorateEdge(edge, source, target, theme, visualSettings, isGroupRelationship, handles)];
  });

  if (parsedEdges.some((edge) => edge.className === "relationship-edge")) return parsedEdges;

  return [...parsedEdges, ...inferGroupRelationships(visibleGroups, groupBounds, theme, visualSettings)];
}
